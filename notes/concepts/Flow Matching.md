...
$\displaystyle \omega \sim \mathcal{N}(0, I)$

step 2. 然后在噪声和真实动作之间构建一个中间状态 noisy action chunk：

$\displaystyle a_{t:t+H}^{\tau,\omega} = \tau a_{t:t+H} + (1-\tau)\omega$
...
$\displaystyle \left\| \omega - a_{t:t+H} - f^a_\theta(a_{t:t+H}^{\tau,\omega}, o_t, \ell) \right\|^2$
...
>
> $\displaystyle f_\theta^a(\text{noisy action}, \text{condition}, \tau) \approx v^\star$
>
> 回归 loss 就是 MSE
>
> $\displaystyle \left\| v^\star - f_\theta^a(\cdot) \right\|^2$

训练目的是让 action expert 学会：当前 $a_{t:t+H}^{\tau,\omega}$ 应该沿着哪个速度方向变化。

最终输出：$v_{\theta}$ ，推理时：

$\displaystyle \frac{d x}{d\tau} = v_\theta(x,\tau,o_t,\hat{\ell})$
